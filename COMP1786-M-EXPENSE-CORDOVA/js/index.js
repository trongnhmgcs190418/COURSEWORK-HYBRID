...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #country').text($('#page-create #frm-register #country').val());
    ...
    $('#page-create #frm-confirm #current_location').text($('#page-create #frm-register #current_location').val());
...
function registerTrip(e) {
    ...
    let country = $('#page-create #frm-register #country').val();
    let description = $('#page-create #frm-register #description').val();
    let current_location = $('#page-create #frm-register #current_location').val();
    let risk = $('#page-create #frm-register #risk').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Current_Location, Destination, Country, Description, Risk, Date) VALUES (?,?, ?, ?, ?, ?, julianday('${date}'))`;
        tx.executeSql(query, [name, current_location, destination, country, description, risk], transactionSuccess, transactionError);
        ...
        function transactionSuccess(tx, result) {
            ...
            $('#page-create #frm-register #country').focus();
            $('#page-create #frm-confirm').popup('close');
        }
    });
}
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #current_location').text(result.rows[0].Current_Location);
                ...
                $('#page-detail #detail #country').text(result.rows[0].Country);
...
function addExpense(e) {
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            let message = `Added expense '${amount.toLocaleString("en-US")} EUR (${type})'.`;
                    ...
                    <div class='list'>
                        <table style='white-space: nowrap; width: 100%;'>
                            <tr style='height: 25px;'>
                                <td>${expense.Type}: ${expense.Amount.toLocaleString('en-US')} EUR</td>
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #current_location`).val(result.rows[0].Current_Location);
                $(`#page-detail #frm-update #destination`).val(result.rows[0].Destination);
                $(`#page-detail #frm-update #country`).val(result.rows[0].Country);
...
function updateTrip(e) {
    ...
    let current_location = $('#page-detail #frm-update #current_location').val();
    let destination = $('#page-detail #frm-update #destination').val();
    let country = $('#page-detail #frm-update #country').val();
    ...
    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Current_Location = ? ,Destination = ?, Country = ?, Description = ?, Risk = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, current_location , country, description, risk, id], transactionSuccess, transactionError);
...
function search(e) {
    ...
    let country = $('#page-list #frm-search #country').val();
    ...
    db.transaction(function (tx) {
        ...
        query += country ? ` Destination LIKE "%${country}%"   AND` : '';
